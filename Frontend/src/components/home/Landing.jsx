...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#0D0B21] text-white selection:bg-violet-500/30 font-sans overflow-x-hidden">
      ...
      {/* Navigation */}
      <nav className="relative z-50 flex items-center justify-between px-10 py-8 container-symmetrical">
        <Link to="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-600 to-fuchsia-600 flex items-center justify-center shadow-lg shadow-purple-500/20 group-hover:scale-110 transition-transform">
            <Code2 size={24} />
          </div>
          <span className="text-2xl font-bold tracking-tight">PrepForge</span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-10 text-sm font-bold text-slate-400 uppercase tracking-widest">
          {user ? (
            <>
              ...
              <Link to="/problems" className="hover:text-white transition-all">Problems</Link>
              <Link to="/streak" className="hover:text-white transition-all">Streak</Link>
              <Link to="/revision" className="hover:text-white transition-all">Revision</Link>
              <Link to="/progress" className="hover:text-white transition-all">Progress</Link>
              <Link to="/profile" className="hover:text-white transition-all">Profile</Link>
            </>
          ) : (
            <>
              <a href="#features" className="hover:text-white transition-all">Features</a>
              <a href="#problems" className="hover:text-white transition-all">Challenges</a>
              <a href="#community" className="hover:text-white transition-all">Community</a>
            </>
          )}
        </div>
        ...
        <div className="flex items-center gap-6">
          {user ? (
            <Link
              to="/profile"
              className="flex items-center gap-3 p-1 pr-4 bg-white/5 border border-white/10 rounded-full hover:border-white/20 transition-all backdrop-blur-md group"
            >
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-600 to-fuchsia-600 flex items-center justify-center text-white text-xs font-bold shadow-lg shadow-purple-500/20 group-hover:scale-105 transition-transform">
                {user.name?.charAt(0).toUpperCase()}
              </div>
              <span className="text-xs font-bold text-white hidden sm:inline">My Profile</span>
            </Link>
          ) : (
            <>
              <Link to="/login" className="text-sm font-bold hover:text-violet-400 transition-colors uppercase tracking-widest">Log in</Link>
              <Link
                to="/signup"
                className="px-8 py-3 bg-white text-[#0D0B21] rounded-full text-sm font-bold hover:bg-violet-50 transition-all shadow-2xl shadow-white/10"
              >
                Get Started
              </Link>
            </>
      ...
      {/* Hero Section */}
      <section className="relative z-10 pt-20 pb-32 container-symmetrical">
        <div className="grid lg:grid-cols-2 gap-20 items-center">
          {/* Left - Text Content */}
          <div className="space-y-8 animate-slide-in">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-violet-400 text-[10px] font-bold uppercase tracking-[0.2em] backdrop-blur-md">
      ...
      {/* Features Grid */}
      <section id="features" className="py-40 relative z-10 container-symmetrical">
        <div className="text-center space-y-4 mb-24">
          <h2 className="text-4xl md:text-6xl font-bold tracking-tight">Built for <span className="gradient-text">Top Engineers</span></h2>
      ...
      <section id="problems" className="py-40 relative z-10 container-symmetrical">
        <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-24">
          <div className="space-y-4">
            <h2 className="text-4xl md:text-6xl font-black tracking-tight">Curated <span className="gradient-text">Challenges</span></h2>
      ...
      <section id="community" className="py-40 relative z-10 container-symmetrical overflow-hidden">
      ...
      {/* CTA */}
      <section className="py-40 container-symmetrical relative overflow-hidden">
